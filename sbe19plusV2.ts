...
export function parseHex(hexFile: string) {
    /*
    ...
    */
    ...
    let sensors = {}, extraSensors = {}, voltages = {}, casts = {};
    ...
    lineReader.on('close', function() {
        ...
        console.log(`   df type: ${df.getColumn('Temperature Frequency').type}, 
        length: ${df.length}, 
        first item: ${df.getColumn('Temperature Frequency').get(0)}, 
        end ${df.getColumn('Temperature Frequency').get(df.count()-1)}`)

        console.log('Processing completed, ' + dataRow + ' records processed, elapsed time: ' + duration.asSeconds() + 's');
        return new Promise(resolve => {
            resolve({"df": df, "casts": casts});
        });
